...
export default function DayEventsModal({ events, dateLabel, onSelect, onAddNew, onClose }) {
  return (
    <div className="fixed inset-0 bg-black/45 flex items-center justify-center z-50 px-4" onClick={onClose}>
      <div className="bg-white rounded-2xl p-5 w-full max-w-xs" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="flex flex-col gap-2">
          ...
          <button
            onClick={onAddNew}
            className="flex items-center gap-2 border border-dashed border-gray-300 rounded-xl px-3 py-2.5 text-sm text-accent"
          >
